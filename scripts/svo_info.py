...
def main():
    ...
    if err == sl.ERROR_CODE.SUCCESS:
        ...
        timestamp = cam.get_timestamp(sl.TIME_REFERENCE.IMAGE)  # Get the image timestamp
        dt = datetime.fromtimestamp(timestamp.get_milliseconds() / 1000.0)
        print("First image time: " + str(dt))
        print("First image size: {0} x {1}\n".format(width, height))
    else:
        print(repr(status))
        exit()
...
def print_camera_information(cam):
    ...
    print("Confidence threshold: {0}".format(
        cam.get_runtime_parameters().confidence_threshold))
    ...
    print("Camera FPS: {0}".format(
        cam.get_camera_information().camera_fps))
    print("Frame count: {0}.".format(cam.get_svo_number_of_frames()))
